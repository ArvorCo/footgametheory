...
def player_table(df: pd.DataFrame, team: str, limit: int = 12) -> str:
    cols = [
        ...
        ("xgi90", "xGI/90"),
        ("pass_security_proxy", "Passe seguro*"),
    ]
    rows = []
    for _, row in df[df["team"] == team].head(limit).iterrows():
        ...
        for key, label in cols:
            ...
            if key.endswith("_index") or key == "overall_index":
                cells.append(f"<td><strong>{fmt(val, 1)}</strong>{bar(float(val), 100, label)}</td>")
            elif key == "pass_security_proxy":
...
def team_comparison(team: pd.DataFrame) -> str:
    ...
    for key, label in metrics:
        ...
        rows.append(
            ...
            f"<td>{html_escape(signed_fmt(edge, 2))}</td>"
            "</tr>"
        )
...
def selected_table(xi: pd.DataFrame) -> str:
    ...
    for _, row in xi.sort_values([POSITION_COL, "overall_index"], ascending=[True, False]).iterrows():
        rows.append(
            ...
            f"<td><strong>{fmt(row['overall_index'], 1)}</strong>{bar(float(row['overall_index']), 100)}</td>"
            f"<td>{html_escape(LANE_LABEL.get(row.get('primary_lane', ''), '-'))}</td>"
            f"<td>{html_escape(THIRD_LABEL.get(row.get('primary_third', ''), '-'))}</td>"
            "</tr>"
        )
    return "<table><thead><tr><th>Titular</th><th>Função</th><th>Min</th><th>Rating</th><th>FGT</th><th>Mapa</th><th>Altura</th></tr></thead><tbody>" + "".join(rows) + "</tbody></table>"
...
def narrative(players: pd.DataFrame, team: pd.DataFrame) -> dict[str, str]:
    ...
    return {
        "headline": (
            ...
            "A partida pede agressão pelo lado forte brasileiro e proteção dura nas costas do meio."
        ),
        "moneyball": (
            f"O maior edge brasileiro é volume de duelos: {pct(float(br['duel_win_rate']))} contra "
            ...
            "Traduzindo: fisicamente o Brasil impõe, mas se o bloco abrir, toma facada."
        ),
        "attack": (
...
def render_report(stats: pd.DataFrame, heatmaps: pd.DataFrame, players: pd.DataFrame, team_match: pd.DataFrame, team: pd.DataFrame, db_relative: str = "build/footgametheory.sqlite") -> str:
    ...
    sub_rows = "".join(
        ...
        f"<td><strong>{html_escape(row['player_label'])}</strong></td>"
        f"<td>{html_escape(row['role'])}</td>"
        f"<td>{fmt(row['overall_index'], 1)}</td>"
        f"<td>{fmt(row['attack_index'], 1)}</td>"
        f"<td>{fmt(row['creation_index'], 1)}</td>"
        f"<td>{fmt(row['defense_index'], 1)}</td>"
    ...
    weak_rows = "".join(
        ...
        f"<td>{html_escape(row['role'])}</td>"
        f"<td>{fmt(row['weighted_rating'], 2)}</td>"
        f"<td>{fmt(row['security_index'], 1)}</td>"
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    section {{ max-width: 1260px; margin: 0 auto 54px; }}
  ...
  <main>
    <section>
      <div class="grid">
        ...
        <div class="card"><div class="label">Duelos vencidos</div><div class="metric">{pct(float(br["duel_win_rate"]))}</div><p>Brasil vence mais contato físico. Use isso sem virar correria burra.</p></div>
        <div class="card"><div class="label">Risco defensivo</div><div class="metric red">{fmt(br["dribbled_past"])}</div><p>Vezes que o Brasil foi driblado; Japão sofreu {fmt(jp["dribbled_past"])}.</p></div>
    ...
    <section class="two">
      ...
      <div class="note">
        <h3>Limitação honesta dos dados</h3>
        <p>O CSV não traz passes totais nem passes errados. Portanto, “passos/passes errados” aqui é medido por proxy: passes certos por toque, perdas por desarme, duelos perdidos e jogador driblado. Não é perfeito, mas é melhor do que inventar número.</p>
    ...
    <section class="three">
      <div class="recommendation">
        ...
        <p><strong>Overload no lado de Vinícius.</strong> Atrair bloco japonês, acelerar com Bruno/Paquetá e atacar o intervalo entre lateral e zagueiro. Vini é o alfa: {fmt(vini["xgi90"], 2)} xGI/90 e {fmt(vini["touches_in_opposition_box90"], 2)} toques na área/90.</p>
      </div>
      <div class="recommendation">
    ...
    <section>
      ...
      <table><thead><tr><th>Jogador</th><th>Função</th><th>FGT</th><th>Ataque</th><th>Criação</th><th>Defesa</th><th>xGI/90</th></tr></thead><tbody>{sub_rows}</tbody></table>
    </section>
    ...
    <section>
      ...
      <table><thead><tr><th>Jogador</th><th>Função</th><th>Rating</th><th>Segurança</th><th>Risco posse</th><th>Driblado/90</th><th>Duelos perdidos/90</th></tr></thead><tbody>{weak_rows}</tbody></table>
    </section>
    ...
    <section>
      ...
      <table><thead><tr><th>Time</th><th>Adversário</th><th>Gols</th><th>xG</th><th>xA</th><th>Chutes</th><th>Chances</th><th>Ações def.</th></tr></thead><tbody>{match_rows}</tbody></table>
    </section>
    ...
    <section>
      ...
      <p>Componentes: ataque (xGI/90, xGOT/90, chutes no alvo, toques na área, dribles), criação (xA/90, chances, passes ao terço final, bolas longas), defesa (ações defensivas, tackles, interceptações, recuperações, duelos), segurança (perdas, duelos perdidos, driblado, passe certo/toque), progressão e goleiro.</p>
